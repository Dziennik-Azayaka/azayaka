...
import { personFormSchema, type PersonFormValues } from '../../forms/person';
...
const emit = defineEmits<{ submit: [PersonFormValues] }>();
...
<template>
  <form @submit="onSubmit" class="space-y-3">
    ...
    <div v-if="initialValues?.pesel || initialValues?.alternateIdentityDocument" class="grid sm:grid-cols-2 gap-3">
      <FormField v-if="initialValues?.pesel" v-slot="{ componentField }" name="pesel">
        <FormItem>
          <FormLabel>{{ t('common.data.pesel') }}</FormLabel>
          <FormControl>
            <Input :disabled="identityReadonly || isPending" v-bind="componentField" />
          </FormControl>
          <FormMessage />
        </FormItem>
      </FormField>
      <FormField v-if="initialValues?.alternateIdentityDocument" v-slot="{ componentField }" name="alternateIdentityDocument">
        <FormItem>
    ...
    <fieldset>
      ...
      <div class="grid sm:grid-cols-2 gap-3">
        ...
        <FormField v-slot="{ componentField }" name="admissionDate">
          <FormItem>
            <FormLabel required>{{ t('common.data.admissionDate') }}</FormLabel>
            <FormControl>
              <Input type="date" :disabled="isPending" v-bind="componentField" />
            </FormControl>
            <FormMessage />
