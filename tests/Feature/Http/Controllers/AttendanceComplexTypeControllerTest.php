...
final class AttendanceComplexTypeControllerTest extends TestCase
{
	...
	public function test_can_list_attendance_complex_types(): void
	{
		...
		$alreadyExistingTypes = AttendanceComplexType::count();
		...
		$jsonNumber = $type->id - 1;
		$response->assertStatus(200);
		$response->assertJsonCount(2 + $alreadyExistingTypes);
		$response->assertJsonPath("$jsonNumber.id", $type->id);
		$response->assertJsonPath("$jsonNumber.name", $type->name);
		$response->assertJsonPath("$jsonNumber.shortcut", $type->shortcut);
		$response->assertJsonPath("$jsonNumber.active", true);
		$response->assertJsonPath("$jsonNumber.primitiveType", $type->maps_to_primitive_type);
	}
	...
	public function test_cannot_create_attendance_complex_type_with_invalid_data(): void
	{
		...
		$this->assertDatabaseCount("attendance_complex_types", $alreadyExistingTypes);
	}
	...
	public function test_update_attendance_complex_type_requires_valid_data(): void
	{
		...
		$this->assertDatabaseCount("attendance_complex_types", 1 + $alreadyExistingTypes);
